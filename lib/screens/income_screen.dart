...
class _IncomeScreenState extends State<IncomeScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        _api.getIncome(year: _selectedYear, month: _selectedMonth, category: _selectedCategory),
        _api.getCategories(),
      ]);
      setState(() {
        _income = results[0] as List<Income>;
        final cats = results[1] as List<Category>;
        _categories = cats.where((c) => c.isIncome).toList();
        _loading = false;
      });
    } catch (e) {
      setState(() {
        _error = 'No se pudo cargar los ingresos.\n$e';
        _loading = false;
      });
    }
  }
  ...
  Widget _buildList() {
    if (_income.isEmpty) {
      return const Center(child: Text('Sin ingresos en este período'));
    }

    final fmt = NumberFormat.currency(locale: 'es_ES', symbol: '€');
    final total = _income.fold(0.0, (s, e) => s + e.amount);
      ...
      children: [
        Container(
          ...
          child: Row(
            ...
            children: [
              Text('${_income.length} ingresos', style: const TextStyle(fontWeight: FontWeight.bold)),
                  ...
                  trailing: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () => _showForm(context, income: e),
                      ),
                      IconButton(
  ...
  Future<void> _showForm(BuildContext context, {Income? income}) async {
...
class _IncomeForm extends StatefulWidget {
  ...
  const _IncomeForm({this.income, required this.categories, required this.onSaved});
...
class _IncomeFormState extends State<_IncomeForm> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (widget.income == null) {
        await _api.addIncome(income);
      } else {
        await _api.updateIncome(widget.income!.row, income);
      }
      widget.onSaved();
      ...
      child: Form(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(
                widget.income == null ? 'Nuevo ingreso' : 'Editar ingreso',
